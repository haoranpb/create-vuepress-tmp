...
module.exports = async function run({ answers, config }) {
  ...
  if (tmpSetting.question) {
    console.log('Template question found, prompting...')
    const tmpAnswsers = await inquirer.prompt(tmpSetting.question)
    Object.assign(answers, tmpAnswsers)
    debug('Merged answers: %O', answers)
  }
